...
public class RegistrySuggestionDropDownWidget extends DropDownWidget<LayoutElement> {
    ...
    public RegistrySuggestionDropDownWidget(
            Registry<?> registry,
            int x,
            int y,
            Consumer<StringOption> setSuggestionFunction,
            Supplier<String> getInputFunction) {
    ...
    protected void buildOptions() {
        ...
        for (var id : this.registry.keySet()) {
            if ((searchMatches || this.search.matches(id.toString(), input))
                    && !this.search.matchesExactly(id.toString(), input)) {
                this.addOption(new StringOption(
                        Component.literal(id.toString()), Minecraft.getInstance().font, setSuggestionFunction));
            }
        }
    }
}
