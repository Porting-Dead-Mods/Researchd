...
public abstract class AbstractResearchEffectListWidget<T extends ResearchEffectList>
    ...
    public AbstractResearchEffectListWidget(int x, int y, T effect) {
        ...
        for (int i = 0; i < effects.size(); i++) {
            ...
            WidgetConstructor<? extends ResearchEffect> widgetConstructor =
                    ResearchdClient.RESEARCH_EFFECT_WIDGETS.get(researchEffect.id());
    ...
    public void setX(int x) {
        ...
        for (AbstractResearchInfoWidget<? extends ResearchEffect> effect : this.effects) {
            effect.setX(x
                    + 1
                    + getSizeFor(this.effects.indexOf(effect), false).width
                    + this.effects.indexOf(effect) * (int) getPadding());
        }
    }
