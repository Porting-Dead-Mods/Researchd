...
public final class SpaghettiClient {
    ...
    public static Set<ResearchNode> getNodesFromScreen() {
        return Minecraft.getInstance().screen instanceof ResearchScreen researchScreen
                ? new HashSet<>(researchScreen
        ...
        mc.execute(() -> {
            if (mc.player != null) {
                mc.player.sendSystemMessage(
                        ResearchdTranslations.component(ResearchdTranslations.Errors.DATA_INCONSISTENCY, message));
            }
        });
    }
}
